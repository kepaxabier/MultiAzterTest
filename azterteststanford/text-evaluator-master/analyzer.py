...
class Analyzer:
    ...
    def sent2sequence(self,sent):
        ...
        for word in sent.words:
                conllword=str(word.index)+"\t"+word.text+"\t"+word.lemma+"\t"+word.upos+"\t"+word.xpos+"\t"+word.feats+"\t"+str(word.governor)+"\t"+str(word.dependency_relation)
                ...
                if not len(word.text) == 1 or word.text.isalpha():
                    if not main_verb_found and int(word.governor) < len(sequence):
                        if self.is_verb(word.upos, int(word.governor), sequence):
                            ...
                            if (word.upos == 'VERB' and word.dependency_relation == 'root') or (word.upos == 'AUX' and wordgovernorvalues[7] == 'root' and wordgovernorvalues[3] == 'VERB'):
    ...
    def is_lexic_word(self, word_upos, word_governor, sequence):
        return self.is_verb(word_upos,word_governor, sequence) or word_upos == 'NOUN' or word_upos == 'ADJ' or word_upos == 'ADV'
    
    def has_modifier(self, entry):
        #nominal head may be associated with different types of modifiers and function words
        return True if entry.dependency_relation in ['nmod', 'nmod:poss', 'appos', 'amod', 'nummod', 'acl', 'acl:relcl', 'det', 'clf', 'case'] else False
    ...
    def count_content_words_in(self, sent):
        ...
        for entry in sent:
            values=entry.split("\t")
            if self.is_verb(values[3],values[6], sent) or values[3] == 'NOUN' or values[3] == 'ADJ' or values[3] == 'ADV':
                num_words += 1
        return num_words
